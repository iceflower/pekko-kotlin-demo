...
class DistributedEventBus private constructor(
    ...
    companion object {
        const val TOPIC_NAME = "spring-sse-events"
    ...
    private val localSubscribers = mutableMapOf<String, (SseEvent) -> Unit>()
    ...
    private fun onSubscribe(cmd: Subscribe): Behavior<Command> {
        localSubscribers[cmd.subscriberId] = cmd.callback
    ...
    private fun onClusterEvent(cmd: ClusterEventReceived): Behavior<Command> {
        when (val event = cmd.event) {
            is BroadcastEvent -> {
                ...
                localSubscribers.values.forEach { callback ->
                    try {
                        callback(sseEvent)
                    } catch (e: Exception) {
                        context.log.warn("구독자에게 이벤트 전달 실패: {}", e.message)
                    }
                }
            }
        }
